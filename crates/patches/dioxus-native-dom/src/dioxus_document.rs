...
use crate::events::{BlitzKeyboardData, BlitzMountedData, NativeClickData, NativeConverter, NativeFormData, make_pixels_rect};
...
pub struct DioxusDocument {
    ...
    /// `onmounted` events collected during the previous mutation pass, deferred
    /// until the start of the next `poll()` so that layout has been computed and
    /// `node.final_layout.size` reflects actual rendered dimensions.
    pub(crate) deferred_mounted_ids: Vec<(ElementId, NodeId)>,
}
...
impl DioxusDocument {
    ...
    pub fn initial_build(&mut self) {
        ...
        // Do NOT drain pending_mounted_ids here. Layout runs AFTER the first poll(),
        // not between initial_build() and poll(). poll() drains pending → deferred
        // after render_immediate; the events fire in the SECOND poll() call, by which
        // time resolve() has computed final_layout.size. Draining here would cause
        // mounted events to fire with final_layout.size = (0, 0).
    }
...
impl Document for DioxusDocument {
    ...
    fn poll(&mut self, cx: Option<TaskContext>) -> bool {
        ...
        // Fire mounted events deferred from the previous poll/initial_build so that
        // node.final_layout.size reflects post-layout dimensions (layout runs between polls).
        let to_fire = std::mem::take(&mut self.deferred_mounted_ids);
        for (element_id, node_id) in to_fire {
            if let Some(node) = self.inner.get_node(node_id) {
                let size = node.final_layout.size;
                let rect = make_pixels_rect(size.width, size.height);
                let data = BlitzMountedData { rect };
                let event = Event::new(
                    std::rc::Rc::new(PlatformEventData::new(Box::new(data)))
                        as std::rc::Rc<dyn std::any::Any>,
                    false,
                );
                self.vdom.runtime().handle_event("mounted", event, element_id);
            }
        }
        ...
        // Defer newly registered onmounted handlers to the next poll (layout runs between).
        let pending = std::mem::take(&mut self.vdom_state.pending_mounted_ids);
        self.deferred_mounted_ids.extend(pending);
