...
impl<'s> SurfaceRenderer<'s> {
    ...
    pub fn maybe_blit_and_present(&self) {
        // PATCH(iris): handle Outdated and Lost gracefully instead of panicking.
        // On Windows/DX12, registering a CustomPaintSource via use_wgpu triggers
        // an extra render cycle during window init that returns Outdated before
        // the swap chain is ready. Skipping the frame is correct — it will retry
        // on the next redraw event. Lost requires surface recreation (handled by
        // the caller on the next frame via resize). See docs/patches.md.
        let surface_texture = match self.surface.get_current_texture() {
            Ok(t) => t,
            Err(wgpu::SurfaceError::Outdated) | Err(wgpu::SurfaceError::Lost) => {
                tracing::warn!(
                    "wgpu_context: surface {:?} — skipping frame",
                    self.surface.get_current_texture().err()
                );
                return;
            }
